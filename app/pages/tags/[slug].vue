  ...
  <UContainer>
    <UPage>
      <UPageBody>
        ...
        <!-- 空状态 -->
        <UAlert
          v-if="!loading && !articles.length && !error && tag"
          color="neutral"
          variant="subtle"
...
const { data: tag, error: tagError } = await useTagBySlug(slug)

if (!tag?.value || !tag.value?.id) {
  throw createError({ status: 404, statusText: '标签不存在' })
}

const tagId = computed(() => tag.value?.id)
...
useSeoMeta({
  ...
  ogImage: () => tag.value?.cover || undefined,
